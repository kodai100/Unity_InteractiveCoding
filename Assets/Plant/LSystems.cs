...
public class LSystems : MonoBehaviour {
    ...
    void Start() {
        ...
        for (int j = 0; j < numberOfPlants; j++) {
            startLocation = new Vector3(50 + Random.Range(-10f, 10f), 0f, 50 + Random.Range(-10f, 10f));
    ...
    void BuildBranch(float angle, Vector3 orientation) {
        ...
        for (int i = 1; i < nSegments; i++) {
            for (int j = 0; j < nn; j++) {
                ...
                prevAng[i][j] = prevAng[i - 1][j] + ang + Random.Range(-branchVariation, branchVariation);
    ...
    Mesh CreateTriMesh(Vector3 a, Vector3 b, Vector3 c) {
        Mesh m = new Mesh();
        m.name = "ScriptedMesh";
        m.vertices = new Vector3[] {a, b, c};
        m.uv = new Vector2[] {
            new Vector2 (0, 0),
            new Vector2 (0, 1),
            new Vector2(1, 1)
