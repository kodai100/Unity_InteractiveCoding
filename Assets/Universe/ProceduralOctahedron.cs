...
public class ProceduralOctahedron : MonoBehaviour {
    ...
    void Start () {
        ...
        Particle[] ps = new Particle[vertices.Length];
        for(int i = 0; i<vertices.Length; i++) {
            ps[i].pos = vertices[i];
            ps[i].uv = uv[i];
        }

        particles = new ComputeBuffer(vertices.Length, Marshal.SizeOf(typeof(Particle)));
        particles.SetData(ps);
    }

    void OnRenderObject() {
        Material m = mat;
        m.SetPass(0);
        m.SetBuffer("_Particles", particles);
        Graphics.DrawProcedural(MeshTopology.Points, particles.count);
    }
