Shader "Custom/ProceduralForm" {
	...
	SubShader{
		...
		v2g vert(uint id : SV_VertexID) {
			...
			OUT.vertex = mul(RotationMatrix(), float4(_Particles[id].pos,1));
			OUT.uv = _Particles[id].uv;	//
		...
		void geom_square(point v2g IN[1], inout TriangleStream<g2f> OUT) {
			...
			if (tex2Dlod(_AlphaMap, float4(IN[0].uv, 0, 0)).a == 0) return;
